...
// "misses" counters.
// Tag = 32 bit - Bit for Index - Bit for Byte
bool Cache::access(unsigned int address) {
  unsigned int indexBits = log2(size/blocksize); // Bit Needed for Index. Set array, Set # = Entries / N- Way = size / blocksize
  unsigned int byteSelect = log2(blocksize);  // Byte Select
  unsigned int tag = address >> (indexBits + byteSelect);  // Note the tag is 32-Bit for Index - Bit for Byte --> we don't need indexbit/byte select --> remove
  unsigned int index = address; // We also need the index to know where to look in the cache for hit/miss. 


  // How do we get index?
  // Move it left by num tag bits to get rid of it. Then move it back
  // then mvoe it right by # for byte select
  // if we move something out of bounds, 0's will replace it on the other end. if we shift back, the values stay 0
  unsigned int numTag = 32 - (indexBits + byteSelect);
  index = index << numTag;
  index = index >> numTag;
  index = index >> byteSelect;


  //We have index in the cache. Now we just check if the tags match.
  // Match? --> Hit
